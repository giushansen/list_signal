defmodule LS.Cache do
  @moduledoc "ETS caches: CTL 5M dedup, HTTP politeness, BGP IP→ASN. No DNS cache."

  use GenServer
  require Logger

  @cache_ttl 1_209_600
  @cleanup_interval 21_600_000
  @ctl_cache :ctl_cache
  ...
  def init(_opts) do
    ...
    schedule_cleanup()
    Logger.info("✅ Cache: CTL 5M + HTTP politeness + BGP IP→ASN")
    {:ok, %{}}
  end
  ...
  def handle_info(:cleanup_cache, state) do
    ...
    total = ctl_del + http_del + bgp_del
    if total > 0, do: Logger.info("🧹 Cache cleanup: #{total} expired (CTL:#{ctl_del} HTTP:#{http_del} BGP:#{bgp_del})")
    schedule_cleanup()
    {:noreply, state}
  end
  ...
  defp ctl_evict do
    ...
    |> Enum.sort_by(fn {_, {_, _, _, ls}} -> ls end)
    |> Enum.take(@eviction_batch)
    |> Enum.each(fn {d, _} -> :ets.delete(@ctl_cache, d) end)
    Logger.info("🧹 CTL eviction: dropped #{@eviction_batch} oldest")
  end

  # === HTTP (politeness — don't re-crawl within 14d) ===
  ...
  def stats do
    http_mem = Float.round((:ets.info(@http_cache, :memory) || 0) * :erlang.system_info(:wordsize) / 1_048_576, 1)
    bgp_mem = Float.round((:ets.info(@bgp_cache, :memory) || 0) * :erlang.system_info(:wordsize) / 1_048_576, 1)
    %{
      ctl: ctl_stats(),
      http: %{entries: :ets.info(@http_cache, :size), memory_mb: http_mem},
      bgp: %{entries: :ets.info(@bgp_cache, :size), memory_mb: bgp_mem}
    }
  end
end

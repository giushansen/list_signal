defmodule LS.ML.Head do
  @moduledoc """
  The trained classification head: a 13-class logistic layer over the MiniLM
  embeddings `LS.ML.Classifier` already computes.

  Weights live in `priv/ml/head_v1.json` — trained 2026-08-14 from 2,200
  Claude-teacher-labeled prod domains (`analysis/distill/`, and
  `ls.ml_teacher_labels` in prod ClickHouse). Unlike the zero-shot cosine it
  replaced, the head's confidence is CALIBRATED: on the owner-labeled golden
  holdout, conf ≥ 0.5 → 68% precision, conf ≥ 0.7 → 91%. The `"Junk"` class
  is part of the softmax but is never emitted as a business model — a
  high-confidence Junk vote means "decline to classify", which keeps this
  module's contract identical to the cosine path it replaces.

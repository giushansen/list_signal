defmodule LSWeb.Layouts do
  ...
  def public_root(assigns) do
    ...
    <html lang="en" class="scroll-smooth">
      <head>
        ...
        <meta property="og:image:alt" content="ListSignal" />
        ...
        <div class="border-t border-white/[0.06] pt-6 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-white/45">
          <div class="text-center md:text-left" itemprop="copyrightHolder" itemscope itemtype="https://schema.org/Organization">
            <p class="flex items-center justify-center md:justify-start gap-2">
              <.logo_mark size={18} class="text-white/35" />
              <span>© 2026 <span itemprop="name">ListSignal</span>. All rights reserved.</span>
            </p>
  ...
  # Favicons, touch icon, web-app manifest and browser theme colour, shared by
  # `public_root/1` and `root/1` so the two layouts cannot drift. The files are
  # generated by docs/brand/gen_brand_assets.py; the SVG comes first so browsers
  # that understand it never download a PNG. theme-color is Tailwind `ls-dark`
  # (assets/tailwind.config.js), the same value the manifest carries.
  defp head_icons(assigns) do
    ~H"""
